...
static void pfilter_output(char *dir, char *fname)
{
	int i;
	int j;
	FILE *f;
	...
	for (i = 0; i < code_pos; i++) {
		for (j = 0; j < 5; j++) {
			fprintf(f, "0x%02x",
				(unsigned)(code_buf[i] >> (8 * (4 - j))) & 0xff);
			if (j != 4)
				fprintf (f, ", ");
			else if (i != code_pos - 1)
				fprintf (f, ",\n");
			else
				fprintf (f, "\n");
		}
...
void pfilter_init_novlan(char *dir, char *fname)
{
	pfilter_new();
	pfilter_nop();
	...
	pfilter_output(dir, fname);

}

void pfilter_init_vlan(char *dir, char *fname)
{
	pfilter_new();
	pfilter_nop();
...
int main(int argc, char **argv) /* no arguments used currently */
{
	...
	pfilter_init_novlan(argv[1], "pfilter-rules-novlan.h");
	pfilter_init_vlan(argv[1], "pfilter-rules-vlan.h");
	exit(0);
}
